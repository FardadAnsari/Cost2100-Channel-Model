%% Demo (clean) – COST 2100: SemiUrban_VLA_2_6GHz + Single link + MIMO_VLA_omni + Wideband
...
% ------------------------
% Scenario parameters (from your original demo)
% ------------------------
...
% ------------------------
% Channel transfer function with 32 omni-directional antennas at BS (λ/2 spacing)
% and one omni-directional MS. Helper returns IR over delay; we FFT to frequency.
delta_f = (freq(2)-freq(1))/256;     % choose 256 tones across the band
...
% RESULTING SHAPE: H_omni_MIMO: [snap, freq, rx, tx] with rx=1, tx=32
...
zlabel('Power [dB]');
title('SemiUrban VLA (User #1, Snapshot #1): |H(f)|^2');
...
ylabel('Avg Power [dB]');
title('Average channel power across frequency');


%ploting channel state information 
figure;
plot((freq(1):delta_f:freq(2))*1e-6, ...
     pow2db(squeeze(mean(mean(abs(H_omni_MIMO).^2, 1), 4))));
xlabel('Frequency [MHz]');
...
% ------------------------
% (Optional) Export CSI for learning
% ------------------------

out.H = H_omni_MIMO;                         % complex CSI
...
save('CSI_export_VLA_single.mat','-struct','out','-v7.3');
save('CSI_axes_meta_VLA_single.mat','axes_info','meta','-v7.3');
fprintf('Saved CSI_export_VLA_single.mat and CSI_axes_meta_VLA_single.mat\n');
